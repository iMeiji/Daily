        ...
        Observable
                .create(new ObservableOnSubscribe<List<ZhuanlanBean>>() {
                    ...
                    public void subscribe(@NonNull ObservableEmitter<List<ZhuanlanBean>> e) throws Exception {
                        e.onNext(InitApp.sDatabase.ZhuanlanNewDao().query(type));
                    }
                })
    ...
    private List<ZhuanlanBean> retrofitRequest() {
        ...
        switch (type) {
            case Constant.TYPE_PRODUCT:
                ids = InitApp.sAppContext.getResources().getStringArray(R.array.product);
                break;
            case Constant.TYPE_MUSIC:
                ids = InitApp.sAppContext.getResources().getStringArray(R.array.music);
                break;
            case Constant.TYPE_LIFE:
                ids = InitApp.sAppContext.getResources().getStringArray(R.array.life);
                break;
            case Constant.TYPE_EMOTION:
                ids = InitApp.sAppContext.getResources().getStringArray(R.array.emotion);
                break;
            case Constant.TYPE_FINANCE:
                ids = InitApp.sAppContext.getResources().getStringArray(R.array.profession);
                break;
            case Constant.TYPE_ZHIHU:
                ids = InitApp.sAppContext.getResources().getStringArray(R.array.zhihu);
                break;
        }
